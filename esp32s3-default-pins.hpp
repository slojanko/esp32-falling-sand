#pragma once

// Avoid and QSPI pins

#define R1_PIN_DEFAULT 1
#define G1_PIN_DEFAULT 2
#define B1_PIN_DEFAULT 42
#define R2_PIN_DEFAULT 41 
#define G2_PIN_DEFAULT 40
#define B2_PIN_DEFAULT 39
#define A_PIN_DEFAULT 0
#define B_PIN_DEFAULT 45
#define C_PIN_DEFAULT 48
#define D_PIN_DEFAULT 47
#define E_PIN_DEFAULT 38 // required for 1/32 scan panels, like 64x64. Any available pin would do, i.e. IO32
#define LAT_PIN_DEFAULT 20
#define OE_PIN_DEFAULT 19
#define CLK_PIN_DEFAULT 21

// #define R1_PIN_DEFAULT 4
// #define G1_PIN_DEFAULT 5
// #define B1_PIN_DEFAULT 6
// #define R2_PIN_DEFAULT 7
// #define G2_PIN_DEFAULT 15
// #define B2_PIN_DEFAULT 16
// #define A_PIN_DEFAULT  18
// #define B_PIN_DEFAULT  8
// #define C_PIN_DEFAULT  3
// #define D_PIN_DEFAULT  42
// #define E_PIN_DEFAULT  -1 // required for 1/32 scan panels, like 64x64. Any available pin would do, i.e. IO32
// #define LAT_PIN_DEFAULT 40
// #define OE_PIN_DEFAULT  2
// #define CLK_PIN_DEFAULT 41
